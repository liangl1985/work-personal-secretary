...
3. **建议配合 Obsidian**：把 `obsidianSyncDir` 指向 vault 里的一个目录（如 `<vault>/work-memory`），插件会把 `MEMORY.md / USER.md / PROJECTS/ / DAILY/ / ARCHIVE/ / GRAPH.json` 单向镜像过去，便于人工翻阅与灾难恢复。**镜像目录请当只读参考**（手改会在下次同步被覆盖，要改请用工具或面板）。
