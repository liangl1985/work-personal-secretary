 ...
 *   7. 转冷待判断提醒：预审拿不准的条目进队列，快照里出现 `⏳ 转冷待判断 N 条`（助手判定，不推给使用者）。
 *
...
export function buildSnapshot({
  ...
  // 6. 转冷待判断提醒（预审拿不准的由助手判定，不整批推给使用者）
  if (triageAsk !== false) {
    try {
      const pend = Object.keys(readTriage(root).pending || {}).length
      if (pend > 0) {
        parts.push({ raw: '⏳ 转冷待判断 ' + pend + ' 条（预审信号不足，需助手判定）：/memory_triage 查看并 keep/cold；超宽限会自动转冷' })
      }
    } catch { /* best-effort */ }
  }
