 ...
 *      **由助手在周保养时逐条判定**（`/memory_triage keep|cold <id>`），
 *      只有助手也拿不准的才交使用者确认。超过宽限期（默认 7 天）仍未判定 → 自然转冷。
 *
...
/**
 * 记录一条"已判定为冷"（助手/使用者拍板）：到期时**不再询问**、直接自然转冷。
 * 与 keep 相对，避免同一条目在每次保养里反复进"待判断"。
 */
export function markCold(state, { id, reason = '', by = 'assistant', today = todayStamp(), label = '', scope = '' }) {
  if (!id) return null
