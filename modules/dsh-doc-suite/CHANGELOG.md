...
- 导出侧原用 `para.text`（纯文本），与生成侧的 `**加粗**` 解析不对称 → 「导出 md → 再生成」**丢加粗**
- 新增 `_runs_md()`：按 run 拼行内 Markdown，加粗 run 包 `**`（标记不紧贴空白）；**表格单元格不加粗**（单元格按 ` | ` 切分、生成侧不做行内解析，写 `**` 会成字面量）
- 实测往返：`**加粗**` 导出 → 再生成 → 加粗 run 保留、正文无星号残留 ✅

### 四、`apply-style` 支持样式颜色（修「公文标题呈主题蓝」）
...
- **安装段收窄**：安装 / 迁移一节由"三条命令 + 长段理由"缩为"**一条主命令 + 参数要点 + 一句不进包理由**"，细节交给 setup_mermaid.ps1 的脚本头注释 —— 技能聚焦"生成素材"，运维细节不占篇幅（146 → **141 行**）
- **顺手修笔误**：设置键示例写成别扭的泛指写法 → 改为具体键名（mediaImageEnabled / mediaArkApiKey）；安装段一处多余反引号
- 验证：技能内容不被测试断言引用，门禁不受影响（media-test 19/0）
...
使用者在集成体的「**文档能力**」页找不到生图设置。核对结论：**该页按设计只放自检面板**（对比「记忆库 24 键 / 专家库 12 键」的形态，见集成体 `RELEASE-CHECKLIST.md` 的验收条），**不渲染插件设置项**；本模块的 `media.*` 属 **DSH 原生设置**（设置 → 插件 → dsh-doc-suite）。`~/.dsh/settings.yaml` 里没有 `dsh-doc-suite` 段**只说明没人改过**，不说明注册失败（DSH 设置面板的数据源是 `settings.describe()`，见 `dsh-api-settings-controller`）。

### 二、改动

1. **`ctx.settings.register(..., { base, applies: 'live' })`** —— 设置改动**免重启生效**（与集成体「记忆库 / 专家库」显示的"改动免重启"一致）；服务端若不认该字段会抛错并被现有 catch 降级，不影响插件启动
2. **`/doc-doctor` 末尾的媒体摘要新增一项**：设置命名空间 `dsh-doc-suite` 已注册（设置 → 插件 → dsh-doc-suite）/ 不可用（已降级为默认值，功能仍可用）—— 让"注册成功与否"**可自查**
3. **`skills/media-gen/SKILL.md`** 新增「设置项在哪儿」一节：指明原生设置入口、说明集成体能力页不含设置项、给出自查方式

### 三、验证

`node --check` 通过 · 五套脚本不变（本次未动 Python 侧）· 改的是 `lib/**` → **需重启 DSH 生效**

### 四、回退

版本改回 **0.7.5**。

## 0.7.5 — 2026-09-16（按官方《快速入门(新手版)》核对密钥口径）

### 一、核对来源

使用者提供的官方 PDF《火山方舟_快速入门(新手版)》（7 页、文本型，2026-09-16 读）。它讲「零基础拿 key 跑通第一个调用」，**不含 API 参数手册**，但确认了两件与本模块直接相关的事：

| 项 | 官方原文口径 | 本模块实现 | 结论 |
|---|---|---|---|
| API Key 格式 | 「当前脚本兼容历史 UUID 格式以及新格式 `ark-<uuid>-<suffix>` 的 Key」 | 按 Bearer 原样使用、不校验格式 | ✅ 兼容（实测用的正是该新格式） |
| 环境变量名 | 「建议先配置 `ARK_API_KEY`，这样后续运行脚本时无需重复输入密钥」；支持「缺少 ARK_API_KEY 时提示输入并写入项目根目录 .env」 | 脚本**优先读 `ARK_API_KEY`**，设置项 `media.ark.api_key` 为备用 | ✅ **完全一致** |
| 模型开通 | 「新用户可使用免费额度…额度使用完毕需要开通模型」；开通 Seedance 2.0 需余额 > 200 元或资源包 | 报 `ModelNotOpen` 即提示到控制台开通；生视频默认关 | ✅ 一致（视频的余额门槛与「默认关视频」的取舍吻合） |

### 二、文档补充（skills/media-gen/SKILL.md）

新增「官方口径」一节：Key 两种格式（新 `ark-<uuid>-<suffix>` / 历史 UUID）· 环境变量名与官方一致 · 三个官方入口（API Key 管理 / 模型开通管理 / 模型广场）· 模型开通与生视频的余额门槛。

### 三、验证

`media-test` 18/0 · 其余四套不变 · 仓库 ↔ profile ↔ 工作区技能三处一致

### 四、回退

版本改回 **0.7.4**。

## 0.7.4 — 2026-09-16（生图云端实测通过 + 落盘格式校验修复）

### 一、实测（本机，事实）

- **调用链一次通过**：命令行 → `gen_image.py` → 方舟 `/api/v3/images/generations`（Bearer 鉴权）
- 第一轮：模型 `doubao-seedream-5-0-pro-260628` · size `1K` · prompt 43 字（**通用信息安全概念插画，不含任何业务信息**）→ 退出码 0 · **42.3 秒** · 产出 **1024×1024 / 213,474 字节**
- 第二轮（格式修复后）：**38.8 秒** · **1024×1024 / 353,145 字节标准 PNG**
- 结论：**55 号记录的端点与模型 ID 均正确**（桌宠项目当年那次走的是第三方代理 `ai.t8star.org` 的 `gpt-image-2`，与本 key 无关；本机现已无该代理凭据）

### 二、密钥口径（本次确认）

- 采用**环境变量**：`ARK_API_KEY` 写入**用户级**环境变量（`HKCU\Environment`，持久、重启后仍在）；脚本优先读它，设置项 `media.ark.api_key` 为备用
- 全程未写入任何文件、未进 git、未在输出中回显（脚本只报「已配置 / 未配置」与长度）

### 三、实测暴露并修复的真实缺口：落盘格式

- **现象**：`--out ark-test.png` 得到 213,474 字节，但读取时报「扩展名声明 png、字节实为其他格式」—— 方舟**返回的实际是 JPEG**
- **修法**（`scripts/media/gen_image.py`）：新增 `detect_image_format()`（按文件头判 PNG/JPEG/GIF/WEBP）与 `_save_image()` —— 按 `--out` 声明的扩展名落盘，**格式不符则用 Pillow 转码**；缺 Pillow 时改存真实扩展名并告警（不再静默产出「假 PNG」）
- **复测**：第二轮输出 **353,145 字节标准 PNG**，日志写明「云端返回 JPEG，已转码为 PNG」
- **回归**：`media-test` 增一条用例（JPEG 字节 → 声明 .png → 断言输出为真 PNG 魔数）

### 四、文档补充

- `skills/media-gen/SKILL.md` 新增「实测基线」：模型可用性、端点、**单张约 40–45 秒**（时间预期）、密钥口径

### 五、验证

`media-test` 17 → **18 例**（新增格式转码用例）；其余四套不变；仓库 ↔ profile ↔ 工作区技能三处一致

### 六、回退

版本改回 **0.7.3**（`scripts/**` 免重启）。
