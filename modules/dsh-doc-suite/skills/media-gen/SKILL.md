...
- 生图 / 生视频的开关、模型、尺寸、密钥都在 **DSH 原生设置**：**设置 → 插件 → `dsh-doc-suite`**（命名空间 `dsh-doc-suite`，键路径 `media.*`）；改动**免重启生效**
...
- 模型默认 `doubao-seedream-5-0-pro-260628`（Seedream 5.0 Pro）**已实测可用**；端点 `https://ark.cn-beijing.volces.com/api/v3/images/generations`（`Authorization: Bearer`）
- 单张 1K 出图**约 40–45 秒**（本机实测 42.3 / 38.8 秒），产出 1024×1024、约 200–350 KB → **时间预期要提前说**
- **云端返回的是 JPEG**：若 `--out` 写成 `.png`，工具会**自动转码**为真 PNG（缺 Pillow 时改存真实扩展名并告警）—— 不要手工改名，否则下游会读到「假 PNG」
- 密钥口径：`ARK_API_KEY` 环境变量（用户级持久即可）优先；未配置时生图不可用并自动回退代码矢量绘制
