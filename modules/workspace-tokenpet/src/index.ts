    ...
    webCtx.effect(() => {
        ...
        try {
          ...
          if (persisted) {
            ...
            const built = buildSessionUsageIndex(sessionQuery, usageIndex, {
              ...
              yieldEvery: 4,
              onProgress: (next) => { if (indexOperation === operation) operation.progress = { ...next, status: 'building' } },
            })
      ...
      disposers.push(ws.register({ kind: 'exact', path: '/workspace-tokenpet/index/build', handler: async (req, res) => {
        ...
        try {
          if (await usageIndex.isPersisted()) {
            if (indexOperation === operation) indexOperation = undefined
            json(res, 409, { error: 'usable index already exists; use /workspace-tokenpet/index/sync' }); return
          }
      ...
      disposers.push(ws.register({ kind: 'exact', path: '/workspace-tokenpet/index/sync', handler: async (req, res) => {
        ...
        try {
          if (!await usageIndex.isPersisted()) {
            if (indexOperation === operation) indexOperation = undefined
            json(res, 409, { error: 'no usable index exists; run /workspace-tokenpet/index/build first' }); return
          }
      ...
      disposers.push(ws.register({
        ...
        handler: async (req, res) => {
          ...
          if (!builtinStripFiles.has(file)) { json(res, 404, { error: 'unknown strip' }); return }
          try {
