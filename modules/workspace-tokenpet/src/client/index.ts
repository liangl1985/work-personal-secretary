/**
 * workspace-tokenpet browser half — mounts a floating "用量小宠物" that is driven by
 ...
 * @module workspace-tokenpet/client
 */
...
function useLifetimeLedger(reloadKey: number, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    fetch('/workspace-tokenpet/usage/lifetime', { signal: request.signal })
...
function useUsageIndex(enabled: boolean): { status: TokenPetIndexStatus | null; progress: IndexProgress; usable: boolean; build: () => void; cancel: () => void } {
  ...
  const poll = useCallback(() => {
    ...
    fetch('/workspace-tokenpet/index/status', { signal: request.signal }).then((r) => r.ok ? r.json() : Promise.reject(new Error('index-status'))).then((value: unknown) => {
  ...
  const build = useCallback(() => {
    ...
    const endpoint = status?.persisted ? '/workspace-tokenpet/index/sync' : '/workspace-tokenpet/index/build'
  ...
  const cancel = useCallback(() => {
    ...
    const request = createPanelRequestScope(PANEL_REQUEST_TIMEOUT_MS)
    operationController.current = request
    fetch('/workspace-tokenpet/index/cancel', { method: 'POST', signal: request.signal }).then(() => poll()).catch(() => {}).finally(() => {
      request.dispose()
      if (operationController.current === request) operationController.current = null
    })
  }, [poll])
...
function TokenPetWindow() {
  ...
  useEffect(() => {
    ...
    fetch(hostUrl('/workspace-tokenpet/skins'), { signal: controller.signal })
      .then(async (response) => {
  ...
  const panelNode = panelOpen ? createPortal(
    h('div', { ref: panelDrag.ref, style: css({ ...panelFixed, width: visibleWidth, height: 'auto', maxHeight: visibleHeight, maxWidth: 'calc(100vw - 16px)', overflow: 'hidden', transform: `translate(${panelDrag.offset.x}px, ${panelDrag.offset.y}px)` }) }, [
      h('div', { style: css(dragHandle), onPointerDown: panelDrag.onPointerDown }, [
        ...
        h('div', { style: css(actionBar) }, [
          h('button', {
            ...
            'aria-controls': 'workspace-tokenpet-prompt-drawer',
      ...
      h('aside', {
        id: 'workspace-tokenpet-prompt-drawer',
...
export function apply(ctx: { slots: SlotsService }): void {
  ...
  `
  if (typeof document !== 'undefined' && document.querySelector('style[data-workspace-tokenpet="1"]') == null) {
  ...
  ctx.slots.inject('settings.section', () => ctx.slots.register(
    { name: 'settings.section', id: 'workspace-tokenpet', order: 60, label: () => h(TokenPetLabel) },
  ...
  ctx.slots.inject('shell.overlay', () => ctx.slots.register(
    { name: 'shell.overlay', id: 'workspace-tokenpet', order: 50, label: () => h(TokenPetLabel) },
    () => h(ErrorBoundary, null, h(TokenPetWindow)),
  ))

  // Session-scoped feed: reports live projection figures to the window.
  ctx.slots.inject('conversation.input.dock', () => ctx.slots.register(
    { name: 'conversation.input.dock', id: 'workspace-tokenpet-feed', order: 50, label: () => h(TokenPetLabel) },
