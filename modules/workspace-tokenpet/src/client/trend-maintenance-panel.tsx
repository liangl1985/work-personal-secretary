...
export function TrendIndexMaintenancePanel({ language: explicitLanguage }: { language?: Language }) {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch('/workspace-tokenpet/usage/trend/status')
      if (!response.ok) {
  ...
  const rebuild = useCallback(async () => {
    ...
    try {
      const response = await fetch('/workspace-tokenpet/usage/trend/repair', { method: 'POST' })
  ...
  const cancel = useCallback(async () => {
    ...
    try {
      const response = await fetch('/workspace-tokenpet/usage/trend/repair/cancel', { method: 'POST' })
      if (!response.ok) {
