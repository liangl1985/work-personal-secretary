...
test('a valid pack is listed with its actions and unresolved files rewritten to URLs', async (t) => {
  ...
  assert.equal(client.animations.idle.file, '/workspace-tokenpet/skins/file?id=lina-pure&file=idle.webp')
  assert.equal(client.animations['tool-success'].file, '/workspace-tokenpet/skins/file?id=lina-pure&file=deep%2Ftool.webp')
  assert.equal(client.preview, '/workspace-tokenpet/skins/file?id=lina-pure&file=preview.webp')
  const packs = await listSkinPacks(home)
  assert.equal(packs.length, 1)
  assert.equal(packs[0].actions.length, 2)
  assert.equal(packs[0].manifestUrl, '/workspace-tokenpet/skins/file?id=lina-pure&file=manifest.json')
})
...
test('skinFileUrl is an exact route carrying id and file as query parameters', () => {
  assert.equal(skinFileUrl('lina-pure', 'idle.webp'), '/workspace-tokenpet/skins/file?id=lina-pure&file=idle.webp')
  assert.equal(skinFileUrl('lina-pure', 'deep/tool.webp'), '/workspace-tokenpet/skins/file?id=lina-pure&file=deep%2Ftool.webp')
})
