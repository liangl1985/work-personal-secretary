 ...
 *
 * 为什么是 2600（2026-09-15 由 2000 上调）：上限写死 4 后实测 4 位精简卡合计
 * 504+542+520+500 = 2066 字符，加 208 字符处理路径提示 ≈ **2363**，已**超过 2000** → 注入层按序降级
 * 到「只留第一位」（729 字符），即「上限 4」在默认预算下形同虚设。2600 给 4 位留出余量
 * （全库最长 4 张合计 2235 + head ≈ 2443）。
 * 注意：预算只是**上限**，命中 1 位时仍只注入 1 位 —— 成本随命中数按需增长，不是恒定增加。
 * 约合 1.7–2.2k token（中文 1 字符 ≈ 0.6–0.7 token）。
 */
export const INJECT_BUDGET_DEFAULT = 2600
