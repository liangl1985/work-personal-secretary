...
**默认一位都不常驻**：任务文本命中关键词时才注入对口 persona（默认 2 位、上限写死 4）；零命中就一位都不注入（宁缺勿滥），改由目录段 + 模型自判 + `expert_recall` 承担。身份由 **work-memory 记忆**承担；确实想常驻一位时，把 `identityExpert` 显式填成某位专家 id。

- **两层命中口径（2026-09-16 使用者定）**：第一层＝关键词命中（插件自动注入对口 persona）；第二层＝**助手有权在实际任务中额外补 1 名专家**（合计不超过「命中数 + 1」），用 `expert_recall` 现取全文即可 —— 这条权限由**目录段首句**每轮告知，不占注入预算、无需新增机制。
...
| **L1 精简卡**（首轮全景） | 角色首句 + 工作方法前 3 条 + 交付与自检前 2 条 + 适用场景（每位约 500–610 字符，方法条 clip 90） | `auto` 首轮：命中专家**全部**注入卡（一个不裁） |
| **L2 全文** | 现有 persona 正文（每位约 1.8–2.6 千字符） | `expert_recall`、**派子代理时内联进 prompt** |

精简卡由正文**确定性生成**（不重写、不修改 `experts/**.md`），所以取全文时内容一字不少；
`expertInjectBudgetChars`（默认 15000，按**最大可能消费**定档）是每轮专家块的**字符上限**，超上限**只截断并标注**（不再降级）：
单篇全文上限 3600 × 位数硬边界 4 + 块头尾注与处理路径 ≈ 14850 ≤ 15000，任何允许的配置组合都不会误触发截断。**绝不静默超限**；20000 是任何形态都不得突破的安全红线。

### 批二：四个注入通道（2026-09-14）
...
| `expertInjectMax` | `4` | **persona 注入软上限**（含身份专家）：**0 = 不限**（交由 `expertInjectBudgetChars` 与分数阈值守门）/ 1 / 2 / 3 / **4**（2026-09-15 上限由 3 放宽到 4；实测 20 条真实任务无一命中 4 位，70% 只命中 1 位，故主要是给复杂任务留余量）；默认形态下每位只占 0.4–0.7 千字符（精简卡），调到 3 时仍受预算约束；工具/技能指针不占该配额 |
| `expertSecondThreshold` | `0.3` | **域专家**第 2/3 位的**相对门槛**：证据 ≥ 本轮最强证据 × 该值才注入（2026-09-15 由 0.8 调为 0.3 —— 相对门槛越高，第一名证据越强时后面越进不来，实测 0.8 时 70% 任务只命中 1 位） |
| `expertGeneralMax` | `1` | **通用型专家**（核查/排版/文档/演示/设计）的独立配额。2026-09-15 起两者**分开评分、分开门槛**：域专家走上面的相对门槛，通用专家走下面的绝对门槛 —— 否则通用专家在 `defaultDomain` 非 general 时拿不到岗位先验，会被结构性挤掉 |
| `expertGeneralMinEvidence` | `0.2` | **通用型专家**的绝对门槛（默认 0.2 = 命中 1 个关键词即可）。通用词更泛、证据天然更低 |
...
| `expertInjectDetail` | `auto` | 注入形态：`auto`（默认，按**会话阶段**分两态 —— 首轮全景给全部命中专家精简卡；干活轮给判定要干活的专家全文，身份专家恒给卡）/ `card`（全部精简卡）/ `full`（全文，旧行为逃生舱，同样受 20000 红线兜底） |
| `expertInjectBudgetChars` | `15000` | 每轮专家注入**字符上限**（2026-09-16 按**最大可能消费**定档：单篇全文上限 3600 × 位数硬边界 4 + 块头尾注与处理路径 ≈ 14850）；**只作上限、不再降级**，超出即截断并标注；下限 200 / 上限 20000（安全红线，任何形态含 `full` 都不得突破） |
...
│   ├── inject.js           # 注入文本组装（目录段 / L1 精简卡 / L2 全文 + 会话阶段两态）
