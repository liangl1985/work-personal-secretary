...
| `expertInjectMax` | `2` | **persona 注入软上限**（含身份专家）：**0 = 不限**（交由 `expertInjectBudgetChars` 与分数阈值守门）/ 1 / 2 / 3 / **4**（2026-09-15 上限由 3 放宽到 4；实测 20 条真实任务无一命中 4 位，70% 只命中 1 位，故主要是给复杂任务留余量）；默认形态下每位只占 0.4–0.7 千字符（精简卡），调到 3 时仍受预算约束；工具/技能指针不占该配额 |
