...
- **`experts/skills.auto.json` 移出版本库**（`git rm --cached` + `.gitignore` 新增规则）：该文件由本机 `scripts/skill-index.mjs` 扫描**本机技能源**生成，**内容含作者机器的绝对路径**；设计上它只是「宿主 `ctx.skills` 注册表不可用时的**可选兜底**」（`lib/store.js:83`；缺文件按空池降级，`ARCHITECTURE.md:120` 已如此描述）。入库并随包会把作者机器路径分发出去，故改为**不入库、需要时本机重建**（`node scripts/skill-index.mjs`）。
  **行为影响：无** —— 首选路径仍是宿主注册表；兜底缺失即返回空池。`scripts/capability-test.mjs` 的对应断言**同时覆盖「存在」与「不存在」两态**，移出后仍 **8/0**。
- **私有称呼 → 「使用者」**：`lib/settings.js:91`（该 `.description()` **会显示在设置页**，属使用者可见文案，优先级最高）· `cordis.patch.yml:31` 注释 · `scripts/regression.mjs:144` · 本文件（7 处历史条目）。
- **私有工作区目录名脱敏**：`lib/match.js:13` 注释里的私有目录名 → `<业务知识库>`。
- **验证**：`regression 48/0` · `injection-tier 20/0` · `capability 8/0` · `coexist 8/0` · `smoke-load 19/0`；`node --check` 改动文件全过。
- **回退**：`git add experts/skills.auto.json` 并删 `.gitignore` 规则，版本改回 0.5.12。
...
- **默认注入位数口径**：`README.md:4` 与 `package.json:4` 原写「默认每轮注入 2 位 / 默认上限 2」，与实现矛盾（`lib/limits.js:12-15` 的 `INJECT_MAX_DEFAULT = 4`）→ 统一为 **4 位**，并标注实现出处。
- **设置项数口径**：`README.md:157` 原写「设置命名空间（16 项）」，实际 DEFAULTS **20 键**、设置页暴露 **19 项** → 已改正并补「除 injectOrder 外免重启」。
- **模块说明版本基线**：`ARCHITECTURE.md:3` 原写 `package.json` 版本 0.5.9（滞后一版）→ 同步为当前版本。
- 来源：2026-09-17 逐插件功能盘点（为随包说明重写而做的独立核对）发现的 6 条文档-实现不一致中的 4 条；另 2 条（桌宠死路由 `strips`、记忆库面板缺「立即备份/归档」按钮）归入 T4 / 单独决定，未在本版处理。
...
**背景**：使用者 2026-09-17 点名做 T6（原话「这个做，你提出的问题特别好」）。8-14 实测的结论是「**数据与内容达标，成品视觉只到勉强能用**」，根因是设计师卡的知识偏 Web/UI，给不出**文档场景的可执行资产**。
...
**背景**：使用者 2026-09-17 提出「避免表述不同但实质一致／不一致的问题一直修」。核查发现本卡仍写「随包内置只有这两个（standard/compact）……正式发文 → `govdoc`（尚未落地）」，而 dsh-doc-suite 0.7.11 已新增 `specs/govdoc.json`（GB/T 9704-2012）—— **卡片与规格不一致**。

- **改法**：风格选型改为**三套内置**（`standard` / `govdoc` / `compact`）+ 公文定位（正式发文，已落地）+ 投标私有口径按需自建自定义层
- **新增单一真相源纪律**：本卡加一句「**版式数值一律以 `specs/*.json` 为准，本卡不重复维护**」—— 防止今后规格改了卡片没改、再次分叉
- **净字数 +3**（改写时同步压缩，未触卡面门禁）
- **验证**：`regression` **48/0** · `card-preview --all` **FAIL 0 · WARN 0 · OK 19**（`general-typeset` 正文 3208 / 卡 503）
- ⚠️ **需重启 DSH**：改的是 `experts/**`（persona 正文）

## 0.5.7 — 2026-09-16（general-slides 补母版与模板约束 · B 线 T5）

**背景**：T5 是 B 线（PPT 能力）的**专家层收口** —— 55 号定的架构是「工具 → 技能 → 专家」，专家要知道**怎么用工具**才有实底。本次只改一位：`general-slides`（演示与汇报设计）。

- **方法 7 改写**（原「套用模板时以模板自身为基线」）→「**套用甲方母版时以母版自身为基线**」：补入**母版盘点**动作（先拿到母版，盘点可用版式与占位符，只在这些版式里排页，**不臆造新骨架**），并保留原有纪律（母版自带的偏差不算我们的问题，产出与它对齐）
- **交付与自检新增两条**：
  - **工具路径**：新建走 `office-ppt` 的 manifest 渲染路径（先校验 → 先样张 → 再整册）；已有 PPT 统一字体走 `apply-style`；配图与图示走 `media-gen` 技能
  - **边界**：命令与参数以技能文档与规格为准，persona 不背命令表；母版盘点结果先写进大纲，再进入排页与生产
- **用词纪律（避撞词）**：`general-designer` 的触发词含「主题 / 样式 / 模板」，本卡一律改用「**母版 / 版式 / 骨架**」，避免同一句话把演示任务判给视觉设计师
- **未动**：`index.json`（母版 / 版式 已在 `general-slides` 的 `trigger_keywords` 内）、`when_to_use`、其他 18 位

### 验证

- 正文 **2202 字**（去空白；门禁区间 900–3000）· L1 卡长 **512**（目标区间 500–620，卡面无截断）
- 五套回归：regression **48/0** · injection-tier **20/0** · capability **8/0** · coexist **8/0** · smoke-load **19/0**（合计 **103/0**）
- `card-preview --all`：**19 位 FAIL 0 · WARN 0**
- 仓库 ↔ profile 逐文件 SHA256 一致

### 需要重启 DSH

改的是 `experts/**`（persona 正文）→ **重启后**新卡才在会话里生效。

### 回退

版本改回 **0.5.6** 或 git revert 本提交；profile 同步一次并重启。
...
- **背景**：使用者 2026-09-15 经四组对照测试后决定「**常规以 4 个专家作为上限**」，判断依据是「4 专家命中场景并不多见」。
...
- **背景**：使用者 2026-09-15 提「通用型专家和域专家评分不应该一样」+「2 到 3 位专家为较为常态」。
...
- **端到端效果**：使用者原句「我需要制作一份给电厂的解决方案，电厂需要对 DCS 和 NCS 进行网络安全建设，等级为等保三级。制作解决方案并在方案中列出需要用到的本公司产品。」—— **改造前 1 位（等保测评）→ 改造后 3 位（工控安全售前 + 等保测评 + 投标与方案策略师）**。
...
- **背景**：使用者判断该卡「用处不大」。实测其 **2930 字中约 1400 字是公文细则与工具细节**（GB/T 9704 纸型版心字号页码、层次序数字体、WPS COM/ProgID、Markdown→公文协议），而本机日常交付走内部材料档（`standard` / `compact`），公文那档几乎不触发；同时**全库无一位专家管「中文文字表述」**（全库检索「表述／润色／措辞／语体／病句」命中皆为他类语境）。
- **定位**：改为**成文的两件事** —— 怎么排（文种、风格、交付形态判断，压成指针）＋ 怎么写（文字表述质量）。**id 与 name 不变**（按使用者要求），`role_tag` 由 `[排版,公文,报告,成文]` 改为 `[排版,报告,成文,表述]`，`when_to_use` 与 `trigger_keywords` 同步换轨（细则词 `字体/行距/红头/GB/T9704/格式` 删除，换成表述类 17 词）。
