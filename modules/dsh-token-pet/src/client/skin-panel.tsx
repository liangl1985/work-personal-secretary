 ...
 *
 * Packs are discovered by the host from `<dsh home>/data/dsh-token-pet/skins/`
 * and listed over `GET /token-pet/skins`, so adding or removing a character is
...
const TEXT = {
  ...
  folderHint: {
    zh: '新增形象：把套装文件夹放到 ~/.dsh/data/dsh-token-pet/skins/<套装id>/（内含 manifest.json 与动作条带），刷新页面即可出现在这里，无需重启。',
    en: 'To add a character, drop a pack folder into ~/.dsh/data/dsh-token-pet/skins/<pack-id>/ (manifest.json plus action strips) and refresh — no restart needed.',
  },
  fallbackNote: {
...
export function SkinImportPanel(p: { onImport?: (bundle: unknown) => void; language?: Language }) {
  ...
  useEffect(() => {
    ...
    fetch('/token-pet/skins', { signal: controller.signal })
      .then(async (response) => {
