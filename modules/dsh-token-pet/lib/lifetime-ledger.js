...
const emptyPayload = (generation = 0) => ({ version: 1, generation, sessions: Object.create(null), floors: Object.create(null) });
const finite = (value) => typeof value === 'number' && Number.isFinite(value) && value >= 0;
const cellTotal = (cells) => cells.reduce((sum, cell) => sum + cell.total, 0);
const cloneCells = (cells) => cells.map(cell => ({ ...cell, totals: { ...cell.totals } }));
...
function validatePayload(value) {
    ...
    if (payload.version !== 1 || !Number.isSafeInteger(payload.generation) || (payload.generation ?? -1) < 0 ||
        !payload.sessions || typeof payload.sessions !== 'object' || !payload.floors || typeof payload.floors !== 'object')
        return undefined;
    ...
    for (const [id, raw] of Object.entries(payload.sessions)) {
        if (!id || !raw || typeof raw !== 'object')
            return undefined;
        const item = raw;
        if (!validFingerprint(item.fingerprint) || typeof item.live !== 'boolean' || !validCells(item.observed) || !validCells(item.credited) || !finite(item.updatedAt))
            return undefined;
    ...
    for (const [id, cells] of Object.entries(payload.floors)) {
        if (!id || !validCells(cells))
            return undefined;
        floors[id] = cloneCells(cells);
    }
    return { version: 1, generation: payload.generation, sessions, floors };
}
function encodedPayload(payload) { return JSON.stringify(payload); }
function checksum(payload) { return createHash('sha256').update(encodedPayload(payload)).digest('hex'); }
function encode(payload) { return JSON.stringify({ ...payload, checksum: checksum(payload) }); }
function decode(raw) {
    ...
    try {
        ...
        if (!payload || typeof document.checksum !== 'string' || document.checksum !== checksum(payload))
            return undefined;
        return payload;
    }
    catch {
...
function liveFingerprintReliable(header) {
...
function mergeMaxCells(previous, current) {
...
function afterFloor(current, floor) {
    const wanted = Math.max(0, cellTotal(current) - cellTotal(floor));
    if (wanted === 0)
        return [];
    ...
    for (const cell of current) {
        ...
        if (total > 0)
            candidates.push({ provider: cell.provider, model: cell.model, day: cell.day, totals, total });
    }
    const available = cellTotal(candidates);
    if (available === wanted)
        return candidates;
    const basis = available > 0 ? candidates : cloneCells(current);
    const basisTotal = Math.max(1, cellTotal(basis));
    let remaining = wanted;
    const out = [];
    for (let index = 0; index < basis.length && remaining > 0; index++) {
        const cell = basis[index];
        const sourceTotal = cell.total;
        const take = index === basis.length - 1 ? remaining : Math.min(remaining, Math.floor(wanted * sourceTotal / basisTotal));
        if (take <= 0)
            continue;
        const names = ['uncachedInputTokens', 'outputTokens', 'cacheReadTokens', 'cacheWriteTokens'];
        const totals = { uncachedInputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 };
        let bucketRemaining = take;
        for (let b = 0; b < names.length; b++) {
            const name = names[b];
            const amount = b === names.length - 1 ? bucketRemaining : Math.min(bucketRemaining, Math.floor(take * cell.totals[name] / Math.max(1, sourceTotal)));
            totals[name] = amount;
            bucketRemaining -= amount;
        }
        out.push({ provider: cell.provider, model: cell.model, day: cell.day, totals, total: take });
        remaining -= take;
    }
    return out;
}
export class FileLifetimeLedger {
    ...
    lockFile;
    recoveryFile;
    constructor(baseDir, fileName = 'lifetime-ledger.json') { this.file = join(baseDir, fileName); this.lockFile = `${this.file}.lock`; this.recoveryFile = `${this.lockFile}.recovery`; }
    /** Atomic rename + checksum make snapshot reads safe without the writer lock. */
    async usage() { const collected = this.collect((await this.load()).sessions); return summarizeUsageCells(collected.cells, collected.sessions); }
    async refresh(sessionQuery, signal, usageIndex) {
        return this.withLock(async () => {
            ...
            for (const record of records) {
                ...
                try {
                    let observed;
                    if (!record.live && usageIndex)
                        observed = await usageIndex.lookup(id, fp);
                    if (!observed) {
                        const source = await sessionQuery.readSession(id);
                        const createdAt = typeof source.session?.createdAt === 'number' ? source.session.createdAt : 0;
    ...
    async clearHistory() {
        return this.withLock(async () => {
            ...
            for (const [id, floor] of Object.entries(state.floors))
                floors[id] = cloneCells(floor);
            for (const [id, entry] of Object.entries(state.sessions))
                floors[id] = mergeMaxCells(floors[id] ?? [], entry.observed);
            const cleared = emptyPayload(state.generation + 1);
            cleared.floors = floors;
            await this.persist(cleared);
            return summarizeUsageCells([]);
        });
    }
    async inspect() { return this.withLock(async () => this.load()); }
    collect(sessions) {
    ...
    async load() {
        ...
        if (invalid)
            throw new Error(`Lifetime Ledger is corrupt and no valid backup is available: ${this.file}`);
        return emptyPayload();
    }
    async persist(state) {
        ...
        try {
            ...
            if (old && decode(old))
                await copyFile(this.file, `${this.file}.bak.tmp`).then(() => rename(`${this.file}.bak.tmp`, `${this.file}.bak`)).catch(async () => { await unlink(`${this.file}.bak.tmp`).catch(() => { }); });
        ...
        while (!handle) {
            ...
            if (await stat(this.recoveryFile).then(() => true).catch(() => false)) {
                if (Date.now() - started > LOCK_WAIT_MS)
                    throw new Error(`lifetime ledger recovery lock timeout: ${this.recoveryFile}`);
                ...
                if (age > LOCK_STALE_MS) {
                    ...
                    if (!lockOwnerAlive(staleOwner)) {
                        ...
                        try {
                            ...
                            if (markerOwner === recoveryOwner)
                                await unlink(this.recoveryFile).catch(() => { });
                        }
                        continue;
                    }
                }
                if (Date.now() - started > LOCK_WAIT_MS)
                    throw new Error(`lifetime ledger lock timeout: ${this.lockFile}`);
                await delay(10 + Math.floor(Math.random() * 20));
            }
        }
...
export function lifetimeLedgerPath(baseDir) { return join(baseDir, 'lifetime-ledger.json'); }
